{{ html()->form('POST', route('laravelblocker::destroy-all-blocked'))
    ...
    @csrf
    @method('DELETE')
    <button class="dropdown-item btn pointer" type="button" style="width: 100%;"
        data-bs-toggle="modal" data-bs-target="#confirm-delete-modal"
        data-title="{{ trans('laravelblocker::laravelblocker.modals.'
                             . 'destroy_all_blocked_title') }}"
        data-message="{!! trans('laravelblocker::laravelblocker.modals.'
                                . 'destroy_all_blocked_message') !!}">
        <i class="fa fa-trash fa-fw" aria-hidden="true"></i>
        {!! trans_choice('laravelblocker::laravelblocker.buttons.destroy-all', 1,
                         ['count' => $blocked->count()]) !!}
    </button>
{{ html()->form()->close() }}

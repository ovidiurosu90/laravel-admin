@php
    ...
    $dataTarget = '#confirm-destroy-roles-modal';
    if(isset($large)) {
        $formClass  = 'mb-0';
    ...
    if($type == 'Permission') {
        ...
        $dataTarget = '#confirm-destroy-permissions-modal';
    }
@endphp
...
<form action="{{ $formAction }}" method="POST" accept-charset="utf-8" data-bs-toggle="tooltip" title="{{ $btnTooltip }}" class="{{ $formClass }}" >
    {{ csrf_field() }}
    {{ method_field('DELETE') }}
    <button class="btn btn-block {{ $btnClass }}" type="button" style="width: 100%;" data-bs-toggle="modal" data-bs-target="{{ $dataTarget }}" data-title="{!! trans('laravelroles::laravelroles.modals.destroy_modal_title', ['type' => $type, 'item' => $item->name]) !!}" data-message="{!! trans('laravelroles::laravelroles.modals.destroy_modal_message', ['type' => $type, 'item' => $item->name]) !!}" >
        {!! $btnText !!}

@php
    ...
    if($type == 'role') {
        ...
        $dataTarget     = '#confirm-restore-roles-modal';
    }
    if($type == 'permission') {
        ...
        $modalTitle     = trans('laravelroles::laravelroles.modals.restore_modal_title', ['type' => $type, 'item' => $item->name]);
        $modalMessage   = trans('laravelroles::laravelroles.modals.restore_modal_message', ['type' => $type, 'item' => $item->name]);
        $dataTarget     = '#confirm-restore-permissions-modal';
    }
    if($style == 'small') {
        $btnClass .= ' btn-sm';
    }
...
<form action="{{ $formAction }}" method="POST" accept-charset="utf-8" data-bs-toggle="tooltip" title="{{ $btnTooltip }}" class="{{ $formClass }}" >
    {{ csrf_field() }}
    {{ method_field('PUT') }}
    <button class="btn btn-block {{ $btnClass }}" type="button" style="width: 100%;" data-bs-toggle="modal" data-bs-target="{{ $dataTarget }}" data-title="{!! $modalTitle !!}" data-message="{!! $modalMessage !!}" >
        {!! $btnText !!}
